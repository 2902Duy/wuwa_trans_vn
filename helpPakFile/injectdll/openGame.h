...
std::optional<std::string> GetOrSelectPath(CSimpleIni& ini, const char* section, const char* name, const char* friendName, const char* filter)
{
	...
	printf("Path %s does not exist. Please find and select Client-Win64-Shipping.exe\n", friendName);
...
bool OpenGameProcess(HANDLE* phProcess, HANDLE* phThread, const char* additionalParam)
{
	...
	{
		std::cout << "Running program with some additional parameters: " << newCommandLine << std::endl;
	}
	else {
		std::cout << "Running program without additional parameters." << std::endl;
	}
	...
	else
	{
		...
		printf("Could not create game process.\n");
		//LOG_ERROR("If you have problem with Client-Win64-Shipping.exe path. You can change it manually in cfg.ini.");
		printf("If you have problems with the Client-Win64-Shipping.exe path, you can change it manually in cfg.ini.\n");
	}
