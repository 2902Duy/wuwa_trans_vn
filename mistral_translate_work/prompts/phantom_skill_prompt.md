...
4. **Nhãn thuộc tính hệ thống**: Giữ nguyên nhãn hệ thống tiếng Anh cho: `Concerto Vibrancy`,  `Echo Skill`, `Resonance Energy`, `DMG`, `HP`, `ATK`, `DEF`.
   - Dòng chứa `Concerto Vibrancy: {x}` bắt buộc giữ nguyên: `Concerto Vibrancy: {x}`.
   - Dòng chứa `Cooldown: {x} seconds` dịch thành: `Thời gian hồi: {x} giây` (hoặc `Hồi chiêu: {x} giây`).
