...
def apply_reverts():
    ...
    for path in sorted(JSON_DIR.rglob("*.json")):
        ...
        for idx, row in enumerate(data):
            ...
            # Check Weapon Conf Name or Weapon Reson Name via key rules (only in name_title domain)
            elif (table == "WeaponConf" and "name_title" in rel_path) or (pk.startswith("WeaponConf") and pk.endswith("_WeaponName")):
                violation_reason = "Weapon Conf Name (Rule 1)"
            elif (table == "WeaponReson" and "name_title" in rel_path) or (pk.startswith("WeaponReson") and pk.endswith("_Name")):
                violation_reason = "Weapon Reson Name (Rule 1)"
            ...
            # If a violation is found, revert the translation
            if violation_reason:
                violations.append({
                    "file": rel_path,
                    "primary_key": pk,
    ...
    print(f"\nApplied reverts for {reverted_count} violations across {modified_files_count} files.")
